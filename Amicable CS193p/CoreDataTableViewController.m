...
@synthesize titleKey, subtitleKey, searchKey, sectionIndexTitlesEnabled;
@synthesize normalPredicate, currentSearchText, searchController;
...
- (NSFetchedResultsController *)fetchedResultsControllerForTableView:(UITableView *)tableView
{
	...
	} else if ((tableView == self.searchDisplayController.searchResultsTableView) && self.searchKey && ![self.currentSearchText isEqual:self.searchDisplayController.searchBar.text]) {
		self.currentSearchText = self.searchDisplayController.searchBar.text;
...
- (void)setFetchedResultsController:(NSFetchedResultsController *)controller
{
	...
	self.normalPredicate = controller.fetchRequest.predicate;
...
{    
	UITableViewCell *cellToReturn = [self tableView:tableView cellForManagedObject:[[self fetchedResultsControllerForTableView:tableView] objectAtIndexPath:indexPath]];
    cellToReturn.textLabel.font = [UIFont fontWithName:@"Marker felt" size:20];
...
- (void)controllerDidChangeContent:(NSFetchedResultsController *)controller
{
    ...
    if (self.sectionIndexTitlesEnabled) {
        [self.tableView reloadData];  // iOS bug workaround (section indexes don't update properly when things change in CoreData)
    }
}
